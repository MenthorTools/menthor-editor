...
import net.menthor.story.ui.menu.*;
...
//		s.getElements().add(w);
		for(TreeItem t : tree.getItems()){
			for(TreeItem t2 : t.getItems()){
				Classification_statement ns = (Classification_statement) t2.getData();
				ns.getHolds_in().remove(w);
				ns.getNot_holds_in().remove(w);
			}
			Object o = t.getData();
...
//		System.out.println("eResource = "+w.eResource());
	...
	public void loadStory(Composite parent) throws IOException{
		...
		dialog.setFilterPath("C:\\Users\\Bernardo\\Documents\\OLED-src\\net.menthor.story\\test_data\\output");//TODO: botar algo mais geral 
	...
	private void saveStory(Composite parent) {
		...
		dialog.setFilterExtensions(new String [] {"*.xmi"});
		dialog.setFilterPath("C:\\Users\\Bernardo\\Documents\\OLED-src\\net.menthor.story\\test_data\\output");
		String result = dialog.open();
		if (result==null)return;
	...
	private Story getStory(){
		...
		//two separate "for" statements so that links can be saved with the correct indexes
		for(TreeItem ti : tree.getItems()){
			if(ti.getData().getClass() == NodeImpl.class){
		...
		for(TreeItem ti : tree.getItems()){
			if(ti.getData().getClass() == LinkImpl.class){
				
				Link l = (Link) ti.getData();
				//
				l.setSource(l.getSource());
				l.setTarget(l.getTarget());
				s.getElements().add(l);
			}
	...
	public TreeItem createNode_state(TreeItem parent, int index){
		TreeItem item = new TreeItem(parent, SWT.CENTER, index);
		Classification_statement n = storyFactory.createClassification_statement();
		
		((Node)parent.getData()).getIs_referred_to_in().add(n);
	...
	public TreeItem addState(TreeItem parent, Classification_statement n, int index){
		TreeItem item = new TreeItem(parent, SWT.CENTER, index);
		item.setData(n);
		((Node)parent.getData()).getIs_referred_to_in().add(n);
		addNewRowWorldButtons(item,n.getLabel()); //adds a row of buttons 
		
		yesWorldButtons(item,n.getHolds_in());
		noWorldButtons(item,n.getNot_holds_in());
		return item;
	}
