...
public class TransferCommander extends GenericCommander {
	...
	private static class TransferLoader {
        private static final TransferCommander INSTANCE = new TransferCommander();
    }	
	public static TransferCommander get() { 
		return TransferLoader.INSTANCE; 
	}	
    private TransferCommander() {
	...
	public void transferNewDataTypes(List<RefOntoUML.Type> newDataTypes){
		for(Element dt: newDataTypes) {
			...
			AddCommand cmd = new AddCommand(MenthorDomain.get().createDomain(), project.getModel().getPackagedElement(), dt);
			MenthorDomain.get().createDomain().getCommandStack().execute(cmd);				
	...
	public void transferConstraints(RefOntoUML.Element element, List<Constraintx> constraints){
		...
		for(Constraintx c: constraints){			
			if (!ProjectUIController.get().getProject().getRefParser().getConstraints(element).contains(c)){				
				toBeAdded.add(c);
			}
		}
		...
		for(Constraintx c: ProjectUIController.get().getProject().getRefParser().getConstraints(element)){
