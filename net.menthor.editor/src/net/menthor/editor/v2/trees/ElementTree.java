...
public class ElementTree extends CheckboxTree {
	...
	public static ElementTree createTree(OntoUMLParser refparser, ElementTreeVisibility opt){
		if(refparser!=null){
			return new ElementTree(new DefaultMutableTreeNode(new RefOntoUMLElement(refparser.getModel(),"")), refparser, opt);
		}else{
			return null;
		}
	}
	
	public static ElementTree createTree(OntoUMLParser refparser, boolean hideDataTypes, ElementTreeVisibility opt){
		if(hideDataTypes) opt.hideDataTypes();
		ElementTree tree = createTree(refparser, opt);
		return tree;
	}
	...
	/**Constructor */
	private ElementTree (DefaultMutableTreeNode rootNode, OntoUMLParser refparser, ElementTreeVisibility opt){	
		this(rootNode);		
